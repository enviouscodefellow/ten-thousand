...
class DumbDarrel(BaseBot):
    def _roll_bank_or_quit(self):
        return 'r'

class QuittingQuitter(BaseBot):
    def _roll_bank_or_quit(self):
        return 'q'

class NaughtyNick(BaseBot):
    def _zilch_chance(self):
        return {1: 2 / 3, 2: 4 / 9, 3: 5 / 18, 4: 17 / 108, 5: 25 / 324, 6: 5 / 216,}[
            self.dice_remaining
        ]
    ...
    def _enter_dice(self):
        ...
        for value in roll:
            roll_string += str(value)
        if GameLogic.calculate_score(roll) < 300:
            self.report("> " + roll_string[0])
            return roll_string[0]
        five_count = 0
        two_count = 0
...
if __name__ == "__main__":
    ...
    DumbDarrel.play(num_games)
    QuittingQuitter.play(num_games)
    NaughtyNick.play(num_games)
